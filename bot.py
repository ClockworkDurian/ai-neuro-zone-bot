...
# Логика для текстового чата (без изменений)
async def handle_text_chat(message: Message):
    ...
    try:
        if provider == "openai":
            ...
            gemini_model = genai.GenerativeModel(model_id); chat_session = gemini_model.start_chat(history=history); response = await chat_session.send_message_async(user_input); answer = response.text; history = [{"role": item.role, "parts": [part.text for part in item.parts]} for item in chat_session.history]
...
# ИСПРАВЛЕНО: Логика для генерации изображений
async def handle_image_generation(message: Message):
    user_id = message.from_user.id
    provider = user_state[user_id].get("provider")
    if not provider:
        await message.answer("Сначала выберите технологию для генерации.")
        return
    ...
    try:
        ...
        if provider == "openai":
            ...
            def _generate_grok_image():
                response = xai_client.image.sample(
                    model="grok-2-image-1212", prompt=prompt, image_format="url"
                )
                return response.url
            image_url = await asyncio.to_thread(_generate_grok_image)
            caption = f"Изображение от Grok Image:\n«{prompt}»"
        
        # Если image_url не пустой, значит, генерация прошла успешно
        if image_url:
            ...
            # Эта ветка сработает, если мы добавим нового провайдера, но забудем реализовать логику
        ...
        # Отправляем пользователю сообщение об ошибке, а не пытаемся отправить фото
...
async def main():
    ...
    # ИСПРАВЛЕНО: Удаляем "зависшие" обновления перед запуском
