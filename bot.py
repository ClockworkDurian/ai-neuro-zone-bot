...
gemini_image_models = {
    "Gemini Image": {"id": "gemini-image-1", "desc": "Отключено"},
}
...
async def show_models(cb):
    ...
    for name, m in models.items():
        text += f"<b>{name}</b> — {m['desc']}\n"
        kb.append([InlineKeyboardButton(text=name, callback_data=f"model:{m['id']}")])

    kb.append([InlineKeyboardButton(text="⬅ Назад", callback_data="back:providers")])
...
async def handle_message(message: types.Message):
    ...
    if st["mode"] == "image":
        ...
        try:
            img = await generate_image(
                st["provider"], st["model"], message.text,
    ...
    try:
        async for chunk in generate_text_stream(
            st["provider"], st["model"], st["history"], message.text,
            OPENAI_API_KEY, GROK_API_KEY, GEMINI_API_KEY, 3000
        ):
            answer += chunk
